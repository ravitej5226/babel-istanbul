...
function usage(arg0, command) {
    ...
    console.error('\nUsage: ' + arg0 + ' ' + command + ' [<options>] <executable-js-file-or-command> [-- <arguments-to-jsfile>]\n\nOptions are:\n\n'
        + [
            ...
            formatOption('--include-babel-polyfill', 'include babel-polyfill, defaults to false'),
            formatOption('--use-babel-runtime', 'uses the optional runtime option for babel, defaults to false'),
            formatOption('--babel-stage <stage>', 'babel stage, defaults to 2')
        ].join('\n\n') + '\n');
    console.error('\n');
}
...
function run(args, commandName, enableHooks, callback) {
    ...
    var template = {
            ...
            'include-pid': Boolean,
            'include-babel-polyfill': Boolean,
            'use-babel-runtime': Boolean,
            'babel-stage': Number,
            i: [ Array, String ]
        },
        ...
        overrides = {
            ...
            instrumentation: {
                ...
                'include-babel-polyfill': opts['include-babel-polyfill'],
                'use-babel-runtime': opts['use-babel-runtime'],
                'babel-stage': opts['babel-stage']
            },
            reporting: {
                reports: opts.report,
                print: opts.print,
    ...
    if (config.instrumentation.includeBabelPolyfill()) {
            ...
            function (err, matchFn) {
                ...
                var coverageVar = '$$cov_' + new Date().getTime() + '$$',
                    instrumenter = new Instrumenter({
                        ...
                        useBabelRuntime: config.instrumentation.useBabelRuntime(),
                        babelStage: config.instrumentation.babelStage()
                    }),
