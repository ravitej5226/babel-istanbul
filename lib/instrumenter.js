...
(function (isNode) {
    ...
    var SYNTAX,
        nodeType,
        BABEL = isNode ? require('babel-core') : null,
        SOURCE_MAP = isNode ? require('source-map') : null,
    ...
    if (!isNode) {
        preconditions = {
            ...
            'Could not find babel-core': BABEL,
            'Could not find source-map': SOURCE_MAP,
    ...
    Instrumenter.prototype = {
        ...
        instrumentSync: function (code, filename, babelConfig) {
            ...
            babelConfig.stage = this.opts.babelStage;

            transformed = BABEL.transform(code, babelConfig);

            this.babelSourceMap = new SOURCE_MAP.SourceMapConsumer(transformed.map);

            program = ESP.parse(transformed.code, {
                loc: true,
                range: true,
        ...
        getPreamble: function (sourceCode, emitUseStrict) {
            ...
            Object.keys(this.coverState.statementMap).forEach(function(key) {
                this.fixLocation(this.coverState.statementMap[key], file);
            }, this);
            ...
            Object.keys(this.coverState.branchMap).forEach(function(key) {
                var branch = this.coverState.branchMap[key];
                branch.locations.forEach(function(loc) {
                    this.fixLocation(loc, file);
                }, this);
